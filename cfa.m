function cfa(destino,filename,filecreat)
clc
aux='\';
...
filecreat=strcat('\',filecreat,'.xls');
filecreat=strcat(destino,filecreat);
    if tam==3
        [T,Tr,Mc]=Modify_ta(Y(:,:,1),Y(:,:,2),Y(:,:,3),PI,TB,RF,def);
    ...
    if tam==4
        ...
        [T,Tr,Mc]=Modify_ta(Positiva,Y(:,:,2),Y(:,:,3),PI,TB,RF,def);        
        copyfile(filename_template,filecreat);
        writetable(Mc,filecreat,'sheet','MCC');
        writetable(T,filecreat,'sheet','CURTO');
        writetable(Tr,filecreat,'sheet','CURTO','Range','I1');
    ...
    if tam>=7
        ...
        Positiva(:,:,1)=Y(:,:,4);Positiva(:,:,2)=Y(:,:,1);
        [Tg,Trg,Mcg]=Modify_ta(Positiva,Y(:,:,2),Y(:,:,3),PI,TB,RF,def);        
        ...
        [T,Tr,Mc]=Modify_ta(Y(:,:,5),Y(:,:,6),Y(:,:,7),PI,TB,RF,def);        
